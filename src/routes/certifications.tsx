...
function CertificationsPage() {
  ...
  useEffect(() => {
    try { const raw = localStorage.getItem(STORAGE_KEY); if (raw) setCerts(JSON.parse(raw)); } catch { /* ignore */ }
  }, []);
  useEffect(() => {
    try { localStorage.setItem(STORAGE_KEY, JSON.stringify(certs)); } catch { /* ignore */ }
  }, [certs]);
