...
function RewardsPage() {
  ...
  useEffect(() => {
    const raw = localStorage.getItem("wis_rewards");
    if (raw) {
      try {
  ...
  useEffect(() => {
    localStorage.setItem("wis_rewards", JSON.stringify({ members, activities }));
  }, [members, activities]);
  ...
  const redeem = (item: RewardItem, memberId: string) => {
    const m = members.find((x) => x.id === memberId);
    if (!m || m.points < item.cost) return;
    setMembers((prev) => prev.map((x) => (x.id === memberId ? { ...x, points: x.points - item.cost } : x)));
    setActivities((prev) => [
